export const languages = {
  en: 'English',
  es: 'Español',
  zh: '简体中文',
};

export const defaultLang = 'en';

export type Lang = keyof typeof ui;

export function resolveLang(locale?: string | null): Lang {
  if (locale && locale in ui) return locale as Lang;
  return defaultLang;
}

export function translateDynamic(t: ReturnType<typeof useTranslations>, key: string): string | undefined {
  return (t as (key: string) => string | undefined)(key);
}

export const ui = {
  en: {
    'nav.home': 'Home',
    'nav.games': 'Games',
    'nav.lifestyle': 'Lifestyle',
    'nav.boredAtWork': 'Bored at Work',
    'nav.boredAtSchool': 'Bored at School',
    'nav.miniGames': 'Mini Games',
    'nav.smallGames': 'Small Games',
    'nav.relaxGames': 'Relax Games',
    'nav.boredGames': 'Bored Games',
    'nav.arcadeGames': 'Arcade Games',
    'nav.waitingInLine': 'Waiting in Line',
    'nav.myGames': 'My Games',
    'nav.blog': 'Blog',
    'hero.play': 'Play Now',
    'hero.surprise': 'Surprise Me!',
    'hero.more': 'Learn More',
    'hero.nowPlaying': 'Now Playing:',
    'hero.details': 'Details',
    'hero.bossKey': 'Boss Key',
    'hero.fullscreen': 'Fullscreen',
    'hero.trending': '🔥 Trending Now (Click to Play Instantly)',
    'game.play': 'Play Game',
    'game.rating': 'Rating',
    'game.category': 'Category',
    'game.favorite': 'Favorite',
    'game.favorited': 'Favorited',
    'game.share': 'Share',
    'game.plays': 'Plays',
    'game.about': 'About',
    'game.description':
      'Play {0} online directly in your browser. No download required. This game is part of the {1} games category.',
    'game.howToPlay': 'How to Play',
    'game.similar': 'Similar Games',
    'game.youMightLike': 'You Might Also Like',
    'game.controls': 'Controls',
    'myGames.title': 'My Games',
    'myGames.favorites': 'Favorites',
    'myGames.history': 'History',
    'myGames.loading': 'Loading your games...',
    'myGames.noFavorites': "You haven't favorited any games yet.",
    'myGames.noHistory': 'No play history yet.',
    'myGames.browse': 'Browse Games',
    'myGames.remove': 'Remove',
    'myGames.playAgain': 'Play Again',
    'footer.about': 'About',
    'footer.terms': 'Terms',
    'footer.privacy': 'Privacy Policy',
    'nav.quickGames': 'Quick Games',
    'nav.hiddenGames': 'Hidden Games',
    'nav.meetingGames': 'Meeting Games',
    'nav.officeGames': 'Office Games',
    'nav.allWorkGames': 'All Work Games',
    'nav.unblockedGames': 'Unblocked Games',
    'nav.chromebookGames': 'Chromebook Games',
    'nav.classroomGames': 'Classroom Games',
    'nav.allSchoolGames': 'All School Games',
    'nav.browserGames': 'Browser Games',
    'nav.html5Games': 'HTML5 Games',
    'nav.noDownloadGames': 'No Download Games',
    'nav.gameTypes': 'Game Types',
    'nav.newGames': 'New Games',
    'nav.contact': 'Contact',
    // Categories
    'cat.arcade': 'Arcade',
    'cat.puzzle': 'Puzzle',
    'cat.strategy': 'Strategy',
    'cat.board': 'Board',
    'cat.card': 'Card',
    'cat.sports': 'Sports',
    'cat.racing': 'Racing',
    'cat.action': 'Action',
    'cat.adventure': 'Adventure',
    'cat.driving': 'Driving',
    'cat.casual': 'Casual',
    'cat.music': 'Music',
    'cat.junior': 'Junior',
    'cat.educational': 'Educational',
    'cat.short': 'Short',
    'cat.stressRelief': 'Stress Relief',
    'cat.calming': 'Calming',
    'cat.classic': 'Classic',
    'cat.retro': 'Retro',
    'cat.modern': 'Modern',
    'blog.title': 'Blog',
    'blog.page': 'Page',
    'blog.subtitle':
      'Discover the best games to play when bored at work or school. Game reviews, top lists, and hidden gems.',
    'blog.headline': 'MiniGamesHub Blog',
    'blog.back': 'Back to Blog',
    'blog.related': 'Related Posts',
    'blog.viewAll': 'View All Posts',
    'blog.minRead': 'min read',
    'blog.category': 'Category',
    'blog.tag': 'Tag',
    'blog.newerPosts': 'Newer posts',
    'blog.olderPosts': 'Older posts',
    'search.title': 'Search',
    'search.description': 'Find your favorite games...',
    'search.placeholder': 'Search games...',
    'search.noResults': 'No games found for [SEARCH_TERM]',
    'search.devOnly': 'Search is available after running npm run build && npm run preview.',
    'cookie.title': 'Cookies & Privacy',
    'cookie.description': 'We use cookies to ensure you get the best experience on our website.',
    'cookie.accept': 'Accept',
    'cookie.decline': 'Decline',
    'cookie.policy': 'Privacy Policy',
  },
  es: {
    'nav.home': 'Inicio',
    'nav.games': 'Juegos',
    'nav.lifestyle': 'Estilo de Vida',
    'nav.boredAtWork': 'Aburrido en el Trabajo',
    'nav.boredAtSchool': 'Aburrido en la Escuela',
    'nav.miniGames': 'Minijuegos',
    'nav.smallGames': 'Juegos Pequeños',
    'nav.relaxGames': 'Juegos de Relax',
    'nav.boredGames': 'Juegos para el Aburrimiento',
    'nav.arcadeGames': 'Juegos Arcade',
    'nav.waitingInLine': 'Esperando en Fila',
    'nav.myGames': 'Mis Juegos',
    'nav.blog': 'Blog',
    'hero.play': 'Jugar Ahora',
    'hero.surprise': '¡Sorpréndeme!',
    'hero.more': 'Leer Más',
    'hero.nowPlaying': 'Jugando:',
    'hero.details': 'Detalles',
    'hero.bossKey': 'Jefe Viene',
    'hero.fullscreen': 'Pantalla Completa',
    'hero.trending': '🔥 Tendencias (Clic para jugar)',
    'game.play': 'Jugar',
    'game.rating': 'Valoración',
    'game.category': 'Categoría',
    'game.favorite': 'Favorito',
    'game.favorited': 'Guardado',
    'game.share': 'Compartir',
    'game.plays': 'Jugadas',
    'game.about': 'Sobre',
    'game.description':
      'Juega a {0} online directamente en tu navegador. Sin descargas. Este juego es parte de la categoría de juegos de {1}.',
    'game.howToPlay': 'Cómo Jugar',
    'game.similar': 'Juegos Similares',
    'game.youMightLike': 'También te puede gustar',
    'game.controls': 'Controles',
    'myGames.title': 'Mis Juegos',
    'myGames.favorites': 'Favoritos',
    'myGames.history': 'Historial',
    'myGames.loading': 'Cargando tus juegos...',
    'myGames.noFavorites': 'Aún no has guardado ningún juego.',
    'myGames.noHistory': 'Aún no hay historial de juego.',
    'myGames.browse': 'Explorar Juegos',
    'myGames.remove': 'Eliminar',
    'myGames.playAgain': 'Jugar de Nuevo',
    'footer.about': 'Sobre nosotros',
    'footer.terms': 'Términos',
    'footer.privacy': 'Política de Privacidad',
    'nav.quickGames': 'Juegos Rápidos',
    'nav.hiddenGames': 'Juegos Ocultos',
    'nav.meetingGames': 'Juegos de Reunión',
    'nav.officeGames': 'Juegos de Oficina',
    'nav.allWorkGames': 'Todos los Juegos de Trabajo',
    'nav.unblockedGames': 'Juegos Desbloqueados',
    'nav.chromebookGames': 'Juegos para Chromebook',
    'nav.classroomGames': 'Juegos de Aula',
    'nav.allSchoolGames': 'Todos los Juegos Escolares',
    'nav.browserGames': 'Juegos de Navegador',
    'nav.html5Games': 'Juegos HTML5',
    'nav.noDownloadGames': 'Juegos Sin Descarga',
    'nav.gameTypes': 'Tipos de Juego',
    'nav.newGames': 'Juegos Nuevos',
    'nav.contact': 'Contacto',
    // Categories
    'cat.arcade': 'Arcade',
    'cat.puzzle': 'Puzzle',
    'cat.strategy': 'Estrategia',
    'cat.board': 'Tablero',
    'cat.card': 'Cartas',
    'cat.sports': 'Deportes',
    'cat.racing': 'Carreras',
    'cat.action': 'Acción',
    'cat.adventure': 'Aventura',
    'cat.driving': 'Conducción',
    'cat.casual': 'Casual',
    'cat.music': 'Música',
    'cat.junior': 'Infantil',
    'cat.educational': 'Educativo',
    'cat.short': 'Cortos',
    'cat.stressRelief': 'Alivio del Estrés',
    'cat.calming': 'Calmantes',
    'cat.classic': 'Clásico',
    'cat.retro': 'Retro',
    'cat.modern': 'Moderno',
    'blog.title': 'Blog',
    'blog.page': 'Página',
    'blog.subtitle':
      'Descubre los mejores juegos para jugar cuando estás aburrido en el trabajo o la escuela. Reseñas de juegos, listas top y joyas ocultas.',
    'blog.headline': 'Blog de MiniGamesHub',
    'blog.back': 'Volver al Blog',
    'blog.related': 'Publicaciones Relacionadas',
    'blog.viewAll': 'Ver Todas',
    'blog.minRead': 'min de lectura',
    'blog.category': 'Categoría',
    'blog.tag': 'Etiqueta',
    'blog.newerPosts': 'Publicaciones Recientes',
    'blog.olderPosts': 'Publicaciones Antiguas',
    'search.title': 'Buscar',
    'search.description': 'Encuentra tus juegos favoritos...',
    'search.placeholder': 'Buscar juegos...',
    'search.noResults': 'No se encontraron juegos para [SEARCH_TERM]',
    'search.devOnly': 'La búsqueda está disponible después de ejecutar npm run build && npm run preview.',
    'cookie.title': 'Cookies y Privacidad',
    'cookie.description': 'Utilizamos cookies para asegurar la mejor experiencia en nuestro sitio web.',
    'cookie.accept': 'Aceptar',
    'cookie.decline': 'Rechazar',
    'cookie.policy': 'Política de Privacidad',
  },
  zh: {
    'nav.home': '首页',
    'nav.games': '游戏',
    'nav.lifestyle': '生活场景',
    'nav.boredAtWork': '上班摸鱼',
    'nav.boredAtSchool': '学校解闷',
    'nav.miniGames': '迷你游戏',
    'nav.smallGames': '小游戏',
    'nav.relaxGames': '放松游戏',
    'nav.boredGames': '无聊必备',
    'nav.arcadeGames': '街机游戏',
    'nav.waitingInLine': '排队解闷',
    'nav.myGames': '我的游戏',
    'nav.blog': '博客',
    'hero.play': '立即开始',
    'hero.surprise': '随机推荐',
    'hero.more': '了解更多',
    'hero.nowPlaying': '正在播放：',
    'hero.details': '详情',
    'hero.bossKey': '老板键',
    'hero.fullscreen': '全屏模式',
    'hero.trending': '🔥 热门推荐 (点击即玩)',
    'game.play': '开始游戏',
    'game.rating': '评分',
    'game.category': '分类',
    'game.favorite': '收藏',
    'game.favorited': '已收藏',
    'game.share': '分享',
    'game.plays': '播放次数',
    'game.about': '关于',
    'game.description': '在浏览器中直接在线玩 {0}。无需下载。本游戏属于 {1} 游戏分类。',
    'game.howToPlay': '游戏玩法',
    'game.similar': '相似游戏',
    'game.youMightLike': '猜你喜欢',
    'game.controls': '控制',
    'myGames.title': '我的游戏',
    'myGames.favorites': '收藏夹',
    'myGames.history': '最近播放',
    'myGames.loading': '正在加载您的游戏...',
    'myGames.noFavorites': '您还没有收藏任何游戏。',
    'myGames.noHistory': '暂无播放记录。',
    'myGames.browse': '浏览游戏',
    'myGames.remove': '移除',
    'myGames.playAgain': '再玩一次',
    'footer.about': '关于我们',
    'footer.terms': '服务条款',
    'footer.privacy': '隐私政策',
    'nav.quickGames': '快节奏游戏',
    'nav.hiddenGames': '隐藏游戏',
    'nav.meetingGames': '会议游戏',
    'nav.officeGames': '办公室游戏',
    'nav.allWorkGames': '所有工作游戏',
    'nav.unblockedGames': '解除封锁游戏',
    'nav.chromebookGames': 'Chromebook 游戏',
    'nav.classroomGames': '教室游戏',
    'nav.allSchoolGames': '所有学校游戏',
    'nav.browserGames': '网页游戏',
    'nav.html5Games': 'HTML5 游戏',
    'nav.noDownloadGames': '无需下载游戏',
    'nav.gameTypes': '游戏类型',
    'nav.newGames': '上新游戏',
    'nav.contact': '联系我们',
    // Categories
    'cat.arcade': '街机',
    'cat.puzzle': '益智',
    'cat.strategy': '策略',
    'cat.board': '棋牌',
    'cat.card': '卡牌',
    'cat.sports': '体育',
    'cat.racing': '赛车',
    'cat.action': '动作',
    'cat.adventure': '冒险',
    'cat.driving': '驾驶',
    'cat.casual': '休闲',
    'cat.music': '音乐',
    'cat.junior': '少儿',
    'cat.educational': '教育',
    'cat.short': '短时',
    'cat.stressRelief': '减压',
    'cat.calming': '解压',
    'cat.classic': '经典',
    'cat.retro': '怀旧',
    'cat.modern': '现代',
    'blog.title': '博客',
    'blog.page': '页',
    'blog.subtitle': '发现在工作或学校感到无聊时最好玩的游戏。游戏评测、榜单和隐藏精品。',
    'blog.headline': 'MiniGamesHub 博客',
    'blog.back': '返回博客',
    'blog.related': '相关文章',
    'blog.viewAll': '查看全部',
    'blog.minRead': '分钟阅读',
    'blog.category': '分类',
    'blog.tag': '标签',
    'blog.newerPosts': '较新文章',
    'blog.olderPosts': '较旧文章',
    'search.title': '搜索',
    'search.description': '寻找您最喜欢的游戏...',
    'search.placeholder': '搜索游戏...',
    'search.noResults': '未找到关于 [SEARCH_TERM] 的游戏',
    'search.devOnly': '搜索功能需先运行 npm run build && npm run preview 后可用。',
    'cookie.title': 'Cookie 与隐私',
    'cookie.description': '我们使用 Cookie 来确保您在我们的网站上获得最佳体验。',
    'cookie.accept': '接受',
    'cookie.decline': '拒绝',
    'cookie.policy': '隐私政策',
  },
  hi: {
    'nav.home': 'होम',
    'nav.games': 'गेम्स',
    'nav.lifestyle': 'लाइफस्टाइल',
    'nav.boredAtWork': 'ऑफिस में बोर हो रहे हैं',
    'nav.boredAtSchool': 'स्कूल में बोर हो रहे हैं',
    'nav.miniGames': 'मिनी गेम्स',
    'nav.smallGames': 'छोटे गेम्स',
    'nav.relaxGames': 'रिलैक्स गेम्स',
    'nav.boredGames': 'बोरियत दूर करने वाले गेम्स',
    'nav.arcadeGames': 'आर्केड गेम्स',
    'nav.waitingInLine': 'लाइन में इंतज़ार करते हुए',
    'nav.myGames': 'मेरे गेम्स',
    'nav.blog': 'ब्लॉग',
    'hero.play': 'अभी खेलें',
    'hero.surprise': 'मुझे चौंकाओ!',
    'hero.more': 'और जानें',
    'hero.nowPlaying': 'अभी चल रहा है:',
    'hero.details': 'विवरण',
    'hero.bossKey': 'बॉस की चाबी',
    'hero.fullscreen': 'फुल स्क्रीन',
    'hero.trending': '🔥 अभी ट्रेंडिंग (क्लिक करें और तुरंत खेलें)',
    'game.play': 'गेम खेलें',
    'game.rating': 'रेटिंग',
    'game.category': 'श्रेणी',
    'game.favorite': 'पसंदीदा',
    'game.favorited': 'पसंदीदा में जोड़ा गया',
    'game.share': 'शेयर करें',
    'game.plays': 'खेलने की संख्या',
    'game.about': 'के बारे में',
    'game.description':
      'अपने ब्राउज़र में सीधे ऑनलाइन {0} खेलें। कोई डाउनलोड नहीं। यह गेम {1} गेम्स श्रेणी का हिस्सा है।',
    'game.howToPlay': 'कैसे खेलें',
    'game.similar': 'समान गेम्स',
    'game.youMightLike': 'ये भी आपको पसंद आ सकते हैं',
    'game.controls': 'कंट्रोल्स',
    'myGames.title': 'मेरे गेम्स',
    'myGames.favorites': 'पसंदीदा',
    'myGames.history': 'इतिहास',
    'myGames.loading': 'आपके गेम्स लोड हो रहे हैं...',
    'myGames.noFavorites': 'आपने अभी तक कोई गेम पसंदीदा में नहीं जोड़ा है।',
    'myGames.noHistory': 'अभी तक कोई प्ले हिस्ट्री नहीं।',
    'myGames.browse': 'गेम्स ब्राउज़ करें',
    'myGames.remove': 'हटाएं',
    'myGames.playAgain': 'फिर से खेलें',
    'footer.about': 'हमारे बारे में',
    'footer.terms': 'नियम और शर्तें',
    'footer.privacy': 'प्राइवेसी पॉलिसी',
    'nav.quickGames': 'क्विक गेम्स',
    'nav.hiddenGames': 'छिपे हुए गेम्स',
    'nav.meetingGames': 'मीटिंग के लिए गेम्स',
    'nav.officeGames': 'ऑफिस गेम्स',
    'nav.allWorkGames': 'सभी वर्क गेम्स',
    'nav.unblockedGames': 'अनब्लॉक्ड गेम्स',
    'nav.chromebookGames': 'क्रोमबुक गेम्स',
    'nav.classroomGames': 'क्लासरूम गेम्स',
    'nav.allSchoolGames': 'सभी स्कूल गेम्स',
    'nav.browserGames': 'ब्राउज़र गेम्स',
    'nav.html5Games': 'HTML5 गेम्स',
    'nav.noDownloadGames': 'बिना डाउनलोड वाले गेम्स',
    'nav.gameTypes': 'गेम टाइप्स',
    'nav.newGames': 'नए गेम्स',
    'nav.contact': 'संपर्क करें',
    'cat.arcade': 'आर्केड',
    'cat.puzzle': 'पज़ल',
    'cat.strategy': 'रणनीति',
    'cat.board': 'बोर्ड',
    'cat.card': 'ताश',
    'cat.sports': 'खेल',
    'cat.racing': 'रेसिंग',
    'cat.action': 'एक्शन',
    'cat.adventure': 'एडवेंचर',
    'cat.driving': 'ड्राइविंग',
    'cat.casual': 'कैज़ुअल',
    'cat.music': 'म्यूज़िक',
    'cat.junior': 'बच्चों के लिए',
    'cat.educational': 'शैक्षणिक',
    'cat.short': 'छोटे',
    'cat.stressRelief': 'स्ट्रेस रिलीफ',
    'cat.calming': 'शांत करने वाले',
    'cat.classic': 'क्लासिक',
    'cat.retro': 'रेट्रो',
    'cat.modern': 'मॉडर्न',
    'blog.title': 'ब्लॉग',
    'blog.page': 'पेज',
    'blog.subtitle':
      'जब आप काम या स्कूल में बोर हों, खेलने के लिए सबसे अच्छे गेम्स खोजें। गेम रिव्यू, टॉप लिस्ट और छिपे हुए रत्न।',
    'blog.headline': 'MiniGamesHub ब्लॉग',
    'blog.back': 'ब्लॉग पर वापस जाएं',
    'blog.related': 'संबंधित पोस्ट',
    'blog.viewAll': 'सभी देखें',
    'blog.minRead': 'मिनट पढ़ने का समय',
    'blog.category': 'श्रेणी',
    'blog.tag': 'टैग',
    'blog.newerPosts': 'नए लेख',
    'blog.olderPosts': 'पुराने लेख',
    'search.title': 'खोजें',
    'search.description': 'अपने पसंदीदा गेम्स ढूंढें...',
    'search.placeholder': 'गेम्स खोजें...',
    'search.noResults': '[SEARCH_TERM] के लिए कोई गेम नहीं मिला',
    'search.devOnly': 'खोज npm run build && npm run preview चलाने के बाद उपलब्ध है।',
    'cookie.title': 'कुकीज़ और प्राइवेसी',
    'cookie.description': 'हम कुकीज़ का उपयोग करते हैं ताकि आपको हमारी वेबसाइट पर सबसे अच्छा अनुभव मिले।',
    'cookie.accept': 'स्वीकार करें',
    'cookie.decline': 'अस्वीकार करें',
    'cookie.policy': 'प्राइवेसी पॉलिसी',
  },
} as const;

export function getLangFromUrl(url: URL) {
  const [, lang] = url.pathname.split('/');
  if (lang in languages) return lang as keyof typeof languages;
  return defaultLang;
}

export function useTranslations(lang: keyof typeof ui) {
  return function t(key: keyof (typeof ui)[typeof defaultLang], params?: (string | number)[]): string {
    let text: string = ui[lang][key] || ui[defaultLang][key];
    if (params && params.length > 0) {
      params.forEach((param, index) => {
        text = text.replace(new RegExp(`\\{${index}\\}`, 'g'), String(param));
      });
    }
    return text;
  };
}

export function getRouteFromUrl(url: URL): string {
  const [, lang, ...rest] = url.pathname.split('/');
  if (lang in languages) {
    return `/${rest.join('/')}`;
  }
  return url.pathname;
}
