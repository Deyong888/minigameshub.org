const categoryGroup = (category, title) => {
  const c = (category ?? "").toLowerCase();
  const t = (title ?? "").toLowerCase();
  const has = (...keys) => keys.some((k) => c.includes(k) || t.includes(k));
...
export { getHowToPlay as a, getTips as b, getGameDetails as c, getGameFaq as d, getHowToPlayZh as e, getTipsZh as f, getCategoryFaq as g, getGameDetailsZh as h, getGameFaqZh as i };
