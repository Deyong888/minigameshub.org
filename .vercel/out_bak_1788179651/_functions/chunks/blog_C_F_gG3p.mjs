...
import { i as AstroError, z as UnknownContentCollectionError, B as RenderUndefinedEntryError, c as createComponent, u as unescapeHTML, a as renderTemplate, C as renderUniqueStylesheet, D as renderScriptElement, G as createHeadAndContent, r as renderComponent } from './astro/server_BWaOESUl.mjs';
import 'piccolore';
import * as devalue from 'devalue';
import { f as cleanSlug, P as POST_PERMALINK_PATTERN, t as trimSlash, h as APP_BLOG, C as CATEGORY_BASE, T as TAG_BASE, B as BLOG_BASE } from './permalinks_DdAc5Pgt.mjs';
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content_BqksRcSt.mjs');
...
const __vite_import_meta_env__ = {"ASSETS_PREFIX": undefined, "BASE_URL": "/", "DEV": false, "MODE": "production", "PROD": true, "SITE": "https://minigameshub.org", "SSR": true};
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_astro_assets_DYW3Dztc.mjs').then(n => n._);
...
const getStaticPathsBlogCategory = async ({ paginate }) => {
  ...
  const enPaths = Array.from(Object.keys(enCategories)).flatMap(
  ...
  const esPaths = Array.from(Object.keys(esCategories)).flatMap(
    (categorySlug) => paginate(
...
const getStaticPathsBlogTag = async ({ paginate }) => {
  ...
  const enPaths = Array.from(Object.keys(enTags)).flatMap(
    (tagSlug) => paginate(
  ...
  const esPaths = Array.from(Object.keys(esTags)).flatMap(
    (tagSlug) => paginate(
...
async function getRelatedPosts(originalPost, maxResults = 4) {
  ...
  const postsWithScores = relevantPosts.reduce((acc, iteratedPost) => {
...
export { getStaticPathsBlogCategory as a, blogListRobots as b, blogCategoryRobots as c, getStaticPathsBlogTag as d, blogTagRobots as e, findPostsByIds as f, getStaticPathsBlogList as g, getRelatedPosts as h, blogPostRobots as i, getStaticPathsBlogPost as j, fetchPosts as k };
